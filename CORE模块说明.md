...
→ FrontendCategory → FrontendApi → ThemeManager
→ oauth/* → Session 启动
```
...
| 公开 API 接口 | `ApiManager` / `ApiNotify` / `ApiProxy` | `FrontendApi` | `admin/api/list.php`、`review.php`、`user/api-manage.php`、`apis.php`（PATH_INFO 网关） | ✅ 是 | **已完成**（本地/外链、`/apis.php/{短码}`、审核三态、投稿与可开关邮件通知） |
...
| `ApiProxy.php` | 外链网关跳转（公开地址 `/apis.php/{短码}`，PATH_INFO，不依赖查询串兜底） |
