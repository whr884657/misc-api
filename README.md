...
<p align="center">
  <img src="https://img.shields.io/badge/version-3.34.0-blue" alt="version">
...
- **积分计费与充值（v3.33+ / v3.34）**：接口收费扣积分；用户充值中心扫码支付；订单管理与积分变动分栏；回调入口 `paynotify.php` / `payreturn.php`
...
├── paynotify.php               # 码支付异步通知入口（→ core/play/codeplay/notify.php）
├── payreturn.php               # 码支付浏览器回跳入口（→ core/play/codeplay/return.php）
...
- 用户充值中心 / 积分变动 UI 重做；码支付回调迁入 `core/play/codeplay`，公网入口 `paynotify.php` / `payreturn.php`（带 `.php`，不改伪静态）
